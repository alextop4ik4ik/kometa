...
local ui = library.new(true, "ꖌ𝙹ᒲᒷℸ ̣ ᔑ ☄☄️ | v"..temptable.version)
ui.ChangeToggleKey(Enum.KeyCode.Semicolon)

local hometab = ui:Category("⍑𝙹ᒲᒷ")
local farmtab = ui:Category("⎓ᔑ∷ᒲ╎リ⊣")
local planterstab = ui:Category("!¡ꖎᔑリℸ ̣ ᒷ∷ᓭ")
local combtab = ui:Category("ᓵ𝙹ᒲʖᔑℸ ̣")
local statstab = ui:Category("ᓭℸ ̣ ᔑℸ ̣ ╎ᓭℸ ̣ ╎ᓵᓭ")
local wayptab = ui:Category("∴ᔑ||!¡𝙹╎リℸ ̣ ᓭ")
local hivetab = ui:Category("⍑╎⍊ᒷ")
local misctab = ui:Category("ᒲ╎ᓭᓵ")
local extrtab = ui:Category("ᒷ ̇/ℸ ̣ ∷ᔑ")
local Configs_Category = ui:Category("ᓵ𝙹リ⎓╎⊣ᓭ")
local setttab = ui:Category("ᓭᒷℸ ̣ ℸ ̣ ╎リ⊣ᓭ")
